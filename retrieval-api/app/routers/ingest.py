"""Upload → ingest endpoint (cloud-native replacement for the UI subprocess call).

In local dev the Nuxt UI shells out to rag-pipeline directly. In the cloud
(UI on Vercel = serverless Node, no Python), that can't work — so the container
that co-locates retrieval-api + rag-pipeline exposes ingestion over HTTP here.
